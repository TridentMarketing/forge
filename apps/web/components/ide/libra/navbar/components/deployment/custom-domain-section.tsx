...
import { Check, Copy, Globe, Loader2, Plus, RefreshCw, Settings, X } from 'lucide-react'
import { useId, useState } from 'react'
import * as m from '@/paraglide/messages'
import type { CustomDomainStatus } from '../../types/deployment'
import { getStatusColor, getStatusText, isApexDomain, useDnsRecords } from './hooks/use-dns-records'
      ...
      <div className='flex items-center gap-3'>
        ...
        <div className='flex-1'>
          <h3 className='text-sm font-semibold text-foreground'>
            {m['ide.deployment.customDomain.customDomain']()}
          </h3>
            ...
            <div className='flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3'>
              <div className='flex items-center gap-3'>
                ...
                <div className='space-y-1 min-w-0'>
                  <h4 className='text-sm sm:text-base font-medium text-foreground break-all'>
                    {customDomainStatus.customDomain}
                  </h4>
                  <div className='flex items-center gap-2'>
                    ...
                    {customDomainStatus.status === 'active' &&
                      customDomainStatus.sslStatus === 'active' && (
            ...
            <div className='space-y-3'>
                      ...
                      <div className='space-y-2'>
                        <div className='flex items-center gap-2 p-2 bg-background rounded border'>
                          <span className='text-xs text-muted-foreground w-12 flex-shrink-0'>
                            {m['ide.deployment.customDomain.name']()}:
                          </span>
                        ...
                        <div className='flex items-center gap-2 p-2 bg-background rounded border'>
                          <span className='text-xs text-muted-foreground w-12 flex-shrink-0'>
                            {m['ide.deployment.customDomain.target']()}:
                          </span>
              ...
              <div
                className={cn(
                  'rounded p-2 text-xs',
                  customDomainStatus.status === 'failed' &&
                    'bg-orange-50 dark:bg-orange-950/20 text-orange-800 dark:text-orange-200',
                  customDomainStatus.status === 'verified' &&
                ...
                isApexDomain(customDomainStatus.customDomain) && (
                  <div className='bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-800 rounded p-2 text-xs text-amber-800 dark:text-amber-200'>
                    <div className='font-medium mb-1'>
                      {m['ide.deployment.customDomain.cnameFlattening']()}
                    </div>
            ...
            >
              <div className='flex items-center justify-center gap-3'>
                ...
                <span className='text-sm font-medium text-foreground'>
                  {m['ide.deployment.customDomain.addCustomDomain']()}
                </span>
              </div>
            </button>
          ) : (
