    ...
    switch (reason) {
      ...
      case 'missing_installation_id':
        return {
          ...
          details:
            'The installation ID was not provided by GitHub. Please try the setup process again.',
        }
      case 'invalid_state':
        return {
          ...
          details:
            'The request state parameter could not be verified. This may be due to an expired or tampered request.',
        }
      case 'no_organization':
        return {
          ...
          details:
            "The GitHub integration requires a valid organization context. Please ensure you're accessing this from within an organization.",
        }
      case 'processing':
        return {
          ...
          details:
            'We encountered an issue while setting up your GitHub integration. Please try again or contact support if the problem persists.',
        }
      case 'missing_code':
        return {
          ...
          details:
            'No authorization code was received from GitHub. Please try the authentication process again.',
        }
      case 'missing_state':
        return {
          ...
          details:
            'The state parameter was not provided. Please start the GitHub integration process again.',
        }
      case 'invalid_nonce':
        return {
          ...
          details:
            'This may be due to a replay attack or an expired request. Please try the integration process again.',
        }
      case 'unexpected':
        return {
          ...
          details:
            "Something went wrong that we didn't anticipate. Please try again or contact our support team for assistance.",
        }
      default:
        return {
      ...
      <Card className='w-full max-w-md'>
        ...
        <CardContent>
          <div className='space-y-4'>
            <p className='text-sm text-muted-foreground text-center'>{errorInfo.details}</p>
            {reason && (
              <div className='p-3 bg-muted rounded-lg'>
                <p className='text-xs font-mono text-muted-foreground'>Error code: {reason}</p>
              </div>
            )}
          </div>
        </CardContent>
        <CardFooter className='flex flex-col gap-3'>
          ...
          <div className='flex gap-2 w-full'>
            <Button variant='outline' onClick={handleCloseWindow} className='flex-1'>
              {isClient && window.opener ? 'Close Window' : 'Go Back'}
            </Button>
            <Link href='mailto:tech@travelresorts.com' className='flex-1'>
